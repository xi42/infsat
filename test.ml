...
let typing_e_test () =
  ...
  [
    ...
    "infer_wo_venv-1" >:: (fun _ ->
        assert_equal_venvls
          [VEnv.empty]
          (typing#infer_wo_venv (hg#find_term (T E)) NP false false)
      );
    ...
    "infer_wo_venv-2" >:: (fun _ ->
        assert_equal_venvls
          []
          (typing#infer_wo_venv (hg#find_term (T E)) NP false true)
      );
    ...
    "infer_wo_venv-3" >:: (fun _ ->
        assert_equal_venvls
          [VEnv.empty]
          (typing#infer_wo_venv (hg#find_term (T E)) NP true false)
      );
  ]
  ...
  [
    "infer_wo_venv-4" >:: (fun _ ->
        assert_equal_venvls
          ...
          (typing#infer_wo_venv (hg#nt_body 1) PR false false)
      );
    ...
    "infer_wo_venv-5" >:: (fun _ ->
        assert_equal_venvls
          []
          (typing#infer_wo_venv (hg#nt_body 1) NP false false)
      );
  ]
  ...
  [
    ...
    "infer_wo_venv-7" >:: (fun _ ->
        assert_equal_venvls
          ...
          (typing#infer_wo_venv (hg#nt_body 4) PR false false)
      );
    ...
    "infer_wo_venv-8" >:: (fun _ ->
        assert_equal_venvls
          ...
          (typing#infer_wo_venv (hg#nt_body 4) NP false false)
      );
  ]
...
let typing_dup () =
  ...
  [
    ...
    "infer_wo_venv-9" >:: (fun _ ->
        assert_equal_venvls
          [
            VEnv.singleton ((2, 0), ity_of_string "pr -> pr");
            VEnv.singleton ((2, 0), ity_of_string "pr -> np");
          ]
          (typing#infer_wo_venv (hg#nt_body 2) PR false false)
      );

    (* No valid environment, because a e is productive and makes the application with it as
       argument productive. *)
    "infer_wo_venv-10" >:: (fun _ ->
        assert_equal_venvls
          []
          (typing#infer_wo_venv (hg#nt_body 2) NP false false)
      );

    (* Only one valid venv when there is a duplication. Since everything in N1 is a variable,
       ...
       known typing of the head with unproductive last argument.
         ...
         * one where a venv did not go through because it did not generate a duplication
           (target pr, head np, pr target args, np actual args as different pr vars) *)
    "infer_wo_venv-11" >:: (fun _ ->
        assert_equal_venvls
          [
            VEnv.of_list [((3, 0), ity_of_string "pr -> pr"); ((3, 1), ity_of_string "pr")];
          ]
          (typing#infer_wo_venv (hg#nt_body 3) PR false false)
      );

    (* Only one valid venv when there is no duplication. This is exactly the opposite of the test
       above with x : pr -> np passing and x : pr -> pr failing and y : pr being forced. *)
    "infer_wo_venv-12" >:: (fun _ ->
        assert_equal_venvls
          [
            VEnv.of_list [((3, 0), ity_of_string "pr -> np"); ((3, 1), ity_of_string "pr")];
          ]
          (typing#infer_wo_venv (hg#nt_body 3) NP false false)
      );
